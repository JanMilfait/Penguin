...
const ChatMessages = ({chat}: {chat: Chat}) => {
  ...
  const { updateScroll, scrollTop } = usePerfectScrollbar(containerRef, {suppressScrollX: true, wheelPropagation: false}, timeout);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      // add 5px tolerance
      if (firstMessagesRendered && container.scrollTop < 5) {
        throttled();
      }
    };
